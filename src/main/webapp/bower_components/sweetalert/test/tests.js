...
test("dismiss modal with confirm-button", function(assert) {
  ...
  var $uibModal = $('.sweet-alert');
  $uibModal.find('button.confirm').click();
  
  setTimeout(function() {
    assert.ok($uibModal.is(':hidden'));
    done();
  }, 500);
});
...
test("cancel-button works", function(assert) {
  ...
  var $uibModal = $('.sweet-alert');
  var $cancelBtn = $uibModal.find('button.cancel');
...
test("timer works", function(assert) {
  ...
  var $uibModal = $('.sweet-alert');
  assert.ok($uibModal.find('button.cancel, button.confirm').is(':hidden'));
...
test("prompt functionality works", function() {
  ...
  ok($uibModal.find('fieldset input').is(':visible'));
  equal($uibModal.find('fieldset input').attr('placeholder'), "Placeholder text");
});
